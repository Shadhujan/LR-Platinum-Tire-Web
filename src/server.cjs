...
let SampleReview = mongoose.model('SampleReview', new mongoose.Schema({
    email1: String,
    username1: String
    ...
    try {
        ...
        const newReview = {
            ...
            reviews: [{
                ...
                image: files.length > 0 ? files.map(file => file.buffer.toString('base64')).join(',') : null,
...
app.get('/Server/Review/Read', async (_req, res) => {
    try {
 ...
 // Update review endpoint (using PUT)
    ...
    try {
        ...
        if (review) {
            ...
            review.image = files.length > 0 ? files.map(file => file.buffer.toString('base64')).join(',') : review.image; // Update the image if new files are uploaded
        }
...
app.delete('/Server/Review/Delete/:reviewId', async (req, res) => {
    ...
    try {
        ...
        console.log(`Looking for Review document with username: ${username} and review ID: ${reviewId}`);
        console.log(Review);
